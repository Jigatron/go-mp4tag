...
func (mp4 MP4) readITCatalogID(boxes MP4Boxes) (int32, error) {
	box := boxes.getBoxByPath("moov.udta.meta.ilst.cnID.data")
	if box == nil {
		return -1, nil
	}
	_, err := mp4.f.Seek(box.StartOffset+16, io.SeekStart)
...
func (mp4 MP4) readITComposerID(boxes MP4Boxes) (int32, error) {
	box := boxes.getBoxByPath("moov.udta.meta.ilst.cmID.data")
	if box == nil {
		return -1, nil
	}
	_, err := mp4.f.Seek(box.StartOffset+16, io.SeekStart)
...
func (mp4 MP4) readITCountryID(boxes MP4Boxes) (int32, error) {
	box := boxes.getBoxByPath("moov.udta.meta.ilst.sfID.data")
	if box == nil {
		return -1, nil
	}
	_, err := mp4.f.Seek(box.StartOffset+16, io.SeekStart)
...
func (mp4 MP4) readITGenreID(boxes MP4Boxes) (int32, error) {
	box := boxes.getBoxByPath("moov.udta.meta.ilst.geID.data")
	if box == nil {
		return -1, nil
	}
	_, err := mp4.f.Seek(box.StartOffset+16, io.SeekStart)
...
func (mp4 MP4) readTags(boxes MP4Boxes) (*MP4Tags, error) {
	...
	catalogID, err := mp4.readITCatalogID(boxes)
	if err != nil {
		return nil, err
	}

	composerID, err := mp4.readITComposerID(boxes)
	if err != nil {
		return nil, err
	}

	countryID, err := mp4.readITCountryID(boxes)
	if err != nil {
		return nil, err
	}

	genreID, err := mp4.readITGenreID(boxes)
	if err != nil {
		return nil, err
	}

	xid, err := mp4.readTag(boxes, "xid ")
	if err != nil {
		return nil, err
	}
	...
	tags := &MP4Tags{
		...
		ItunesCountryID:  countryID,
		ItunesComposerID: composerID,
		ItunesGenreID:    genreID,
		Lyrics:           lyrics,
		Narrator:         narrator,
