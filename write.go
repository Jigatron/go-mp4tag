...
func overwriteTags(mergedTags, tags *MP4Tags, delStrings []string) *MP4Tags {
	...
	if containsStr(delStrings, "itunescatalogid") {
		mergedTags.ItunesCatalogID = 0
	}

	if containsStr(delStrings, "itunescomposerid") {
		mergedTags.ItunesComposerID = 0
	}

	if containsStr(delStrings, "itunescountryid") {
		mergedTags.ItunesCountryID = 0
	}

	if containsStr(delStrings, "itunesgenreid") {
		mergedTags.ItunesGenreID = 0
	}
	...
	if tags.ItunesCatalogID > 0 {
		mergedTags.ItunesCatalogID = tags.ItunesCatalogID
	}

	if tags.ItunesComposerID > 0 {
		mergedTags.ItunesComposerID = tags.ItunesComposerID
	}

	if tags.ItunesCountryID > 0 {
		mergedTags.ItunesCountryID = tags.ItunesCountryID
	}

	if tags.ItunesGenreID > 0 {
		mergedTags.ItunesGenreID = tags.ItunesGenreID
	}
...
func writeItunesCatalogID(f *os.File, catalogID int32) error {
	_, err := f.Write([]byte{0x0, 0x0, 0x0, 0x1C})
	if err != nil {
		return err
	}
	_, err = f.WriteString("cnID")
	if err != nil {
		return err
	}
	_, err = f.Write([]byte{0x0, 0x0, 0x0, 0x14})
	...
	catalogIDBytes := putI32BE(catalogID)
	_, err = f.Write(catalogIDBytes)
	return err
}

func writeItunesComposerID(f *os.File, composerID int32) error {
	_, err := f.Write([]byte{0x0, 0x0, 0x0, 0x1C})
	if err != nil {
		return err
	}
	_, err = f.WriteString("cmID")
	if err != nil {
		return err
	}
	_, err = f.Write([]byte{0x0, 0x0, 0x0, 0x14})
	...
	composerIDBytes := putI32BE(composerID)
	_, err = f.Write(composerIDBytes)
	return err
}

func writeItunesCountryID(f *os.File, countryID int32) error {
	_, err := f.Write([]byte{0x0, 0x0, 0x0, 0x1C})
	if err != nil {
		return err
	}
	_, err = f.WriteString("sfID")
	if err != nil {
		return err
	}
	_, err = f.Write([]byte{0x0, 0x0, 0x0, 0x14})
	...
	countryIDBytes := putI32BE(countryID)
	_, err = f.Write(countryIDBytes)
	return err
}

func writeItunesGenreID(f *os.File, genreID int32) error {
	_, err := f.Write([]byte{0x0, 0x0, 0x0, 0x1C})
	if err != nil {
		return err
	}
	_, err = f.WriteString("geID")
	if err != nil {
		return err
	}
	_, err = f.Write([]byte{0x0, 0x0, 0x0, 0x14})
	...
	genreIDBytes := putI32BE(genreID)
	_, err = f.Write(genreIDBytes)
	return err
}
...
func (mp4 MP4) writeTags(boxes MP4Boxes, tags *MP4Tags, tempPath string) error {
	...
	if tags.ItunesCatalogID > 0 {
		err = writeItunesCatalogID(f, tags.ItunesCatalogID)
		if err != nil {
			return err
		}
	}
	...
	if tags.ItunesComposerID > 0 {
		err = writeItunesComposerID(f, tags.ItunesComposerID)
		if err != nil {
			return err
		}
	}
	...
	if tags.ItunesCountryID > 0 {
		err = writeItunesCountryID(f, tags.ItunesCountryID)
		if err != nil {
			return err
		}
	}
	...
	if tags.ItunesGenreID > 0 {
		err = writeItunesGenreID(f, tags.ItunesGenreID)
		if err != nil {
			return err
		}
	}
	...
	if tags.XID != "" {
		err = writeRegular(f, "xid ", tags.XID, false)
		if err != nil {
			return err
		}
	}
